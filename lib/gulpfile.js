...
function compile(modules, processLess = true, processLocale = true) {
  const {
    compile: { transformTSFile, transformFile, transformLessFile, includeLessFile = [] } = {},
  ...
  if (processLess) {
    less = gulp
      .src([`${componentDir}/**/*.less`])
      .pipe(
        through2.obj(function (file, encoding, next) {
          // Replace content
          let cloneFile = file.clone();
          const content = file.contents.toString().replace(/^\uFEFF/, '');

          cloneFile.contents = Buffer.from(content);

          if (transformLessFile) {
            cloneFile = transformLessFile(cloneFile) || cloneFile;
          }
  ...
  const source = [
    ...
    `${componentDir}/**/*.ts`,
    'typings/**/*.d.ts',
    `!${componentDir.replace(/^\.\//, '')}/**/__tests__/**`,
    `!${componentDir.replace(/^\.\//, '')}/**/demo/**`,
  ];
  ...
  if (tsConfig.allowJs) {
    source.unshift(`${componentDir}/**/*.{js,jsx}`);
  }
  ...
  const tsResult = sourceStream.pipe(
    ts(packageJson.name === 'gdcd' ? tsConfig : ts.createProject(getProjectPath('tsconfig.json')), {
      error(e) {
...
gulp.task(
  ...
  gulp.series(done => {
    ...
    checkDiff(packageJson.name, packageJson.version, registry, done);
  })
);
